...
#   user = Scrobbler2::User.new('jnunemaker')
# 
...
module Scrobbler2
  class User < Base
    ...
    def neighbours(params={},force=false)
      params = params.merge(:user => @username)
      get_instance("user.getneighbours", params, :neighbours, :user, force)
    end
    
    def top_artists(params={},force=false)
      params = params.merge(:user => @username)
      get_instance("user.gettopartists", params, :top_artists, :artist, force)
    end
    
    def library(params={},force=false)
      if @library.nil? || force
        params = params.merge(:user => @username)
        doc = User.fetch_and_parse("library.getartists",params)
        total = doc.at(:artists)['totalPages']
        library = (doc/:artist).inject([]) do |elements, el| 
          elements << Artist.new_from_xml(el, doc)
          elements
        end
        (total.to_i-1).times do |n|
          params[:page] = (n + 2)
          library << get_library_page(params)
        end
        @library = library.flatten
      else
        @library
      end
    end
    
    def get_library_page(params)
      doc = User.fetch_and_parse("library.getartists",params)
      elements = (doc/:artist).inject([]) do |elements, el| 
        elements << Artist.new_from_xml(el, doc)
        elements
      end
      elements
